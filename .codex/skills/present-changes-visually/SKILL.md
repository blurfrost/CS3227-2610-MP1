...
4. Run the bundled generator from the installed skill directory, with the
   repository root as the target working directory:

   ```bash
   python3 <installed-skill-directory>/scripts/generate-split-view-diff.py \
     . HEAD WORKTREE _temp/visual-diff.html
   ```

   Resolve `<installed-skill-directory>` to the directory containing this
   skill, then replace `HEAD`, `WORKTREE`, and the output path with the
   requested values. The comparison points can be any Git commit-ish such as
   `HEAD~1`, a tag, a branch, or a commit SHA. Use `WORKTREE` for the current
   files.
